...
- `asHaiaPolicyError(err)` — распознавание блока сквозь обёртки стека: viem заворачивает ошибку транспорта в `TransactionExecutionError`, поэтому голый `instanceof` у партнёра не сработает.
- Вердикт валидируется: 200 с нераспознанным телом трактуется как недоступность и уходит в fail-mode, а не как разрешение.
