...
**What the client does with them.** Every one of them produces the same fail-mode fallback as an outage does — the class of the action decides — with the code itself added to `reasons`, so a misconfigured stand can be told from an engine outage in your telemetry. A configuration code no longer counts toward the circuit breaker (every call would answer the same), a rate limit shuts the gate for the `Retry-After` the engine asked for, up to a minute, and a configuration code writes the engine's own message to the console once per code: on a 502 it names the stream nothing is deployed for, which is the thing to go and fix. A transient outage stays silent, as it was before — its message says nothing a retry would not.
